...
MyDomain = "https://myplex.doussis.be"
Port = "32400"
Serveur = "MyPlex"
NomUser = "SaigoNoo"
MotDePasse = "pass"
...
if ( DesktopMode == "true" ):
	os.startfile("C:\Program Files\Plex\Plex\Plex.exe")
else:
	webbrowser.open(MyDomain+":"+Port)
...
class discordRichPresence:
	...
	def stop(self):
		if (("Plex.exe" in (p.name() for p in psutil.process_iter())) == False and DesktopMode == "true" ):
			os.system("taskkill /f /im pythonw" + pyVersion + ".exe")
		if (DesktopMode == "false" and ("firefox.exe" in (p.name() for p in psutil.process_iter())) == False):
			os.system("taskkill /f /im pythonw" + pyVersion + ".exe")
		if (DesktopMode == "false" and ("Chrome.exe" in (p.name() for p in psutil.process_iter())) == False):
			os.system("taskkill /f /im pythonw" + pyVersion + ".exe")
		if (DesktopMode == "false" and ("Opera.exe" in (p.name() for p in psutil.process_iter())) == False):
			os.system("taskkill /f /im pythonw" + pyVersion + ".exe")
